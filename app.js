...
`;
    ...
    loadPDFLibrary(function () {
        ...
        bukitBarisArr = Array.from(bukitBarisSet)
            ...
            .sort((a, b) => {
                const compBukit = (parseInt(a.bukit) || a.bukit).toString().localeCompare((parseInt(b.bukit) || b.bukit).toString(), 'id', { numeric: true });
                if (compBukit !== 0) return compBukit;
                return (parseInt(a.baris) || a.baris).toString().localeCompare((parseInt(b.baris) || b.baris).toString(), 'id', { numeric: true });
            });
        pokokArr = Array.from(pokokSet)
            .sort((a, b) => (parseInt(a) || a).toString().localeCompare((parseInt(b) || b).toString(), 'id', { numeric: true }));

        // --- Tambah NOTE ke cellData dan header ---
        const cellData = {};
        blokDataArr.forEach(bd => {
            bd.pokok_sampel.forEach(pokok => {
                const k = `${pokok.no_pokok}_${bd.no_bukit}_${bd.no_baris}`;
                let kary = `${pokok.bunga_betina ?? '-'} | ${pokok.bunga_jantan ?? '-'}`;
                let karyNote = pokok.note || '';
                let ast = bd.verifikasi?.ast?.[pokok.no_pokok];
                let askep = bd.verifikasi?.askep?.[pokok.no_pokok];
                let astAskep = '';
                if (ast && ((parseInt(ast.bunga_betina) > 0) || (parseInt(ast.bunga_jantan) > 0))) {
                    astAskep = `${ast.bunga_betina ?? ''} | ${ast.bunga_jantan ?? ''}`;
                } else if (askep && ((parseInt(askep.bunga_betina) > 0) || (parseInt(askep.bunga_jantan) > 0))) {
                    astAskep = `${askep.bunga_betina ?? ''} | ${askep.bunga_jantan ?? ''}`;
                }
                let em = bd.verifikasi?.em?.[pokok.no_pokok];
                let rc = bd.verifikasi?.rc?.[pokok.no_pokok];
                let emRc = '';
                if (em && ((parseInt(em.bunga_betina) > 0) || (parseInt(em.bunga_jantan) > 0))) {
                    emRc = `${em.bunga_betina ?? ''} | ${em.bunga_jantan ?? ''}`;
                } else if (rc && ((parseInt(rc.bunga_betina) > 0) || (parseInt(rc.bunga_jantan) > 0))) {
        ...
        for (let colStart = 0; colStart < bukitBarisArr.length; colStart += maxSetPerPage) {
            ...
            while (bukitBarisSub.length < maxSetPerPage) {
                bukitBarisSub.push({ bukit: '-', baris: '-' });
            }

            addCustomHeaderPortrait(doc, company, estateName, estate, blok, blokDataArr, bukitBarisSub, pageIdx, Math.ceil(bukitBarisArr.length / maxSetPerPage));

            // --- HEADER dengan Kary Note ---
            let headRows = [
                [
                    { content: 'No. Pokok', rowSpan: 2 },
                    ...bukitBarisSub.map(bb => ({
                        content: (bb.bukit !== '-' ? `No. Bukit: ${bb.bukit}\nNo. Baris: ${bb.baris}` : ''),
                        colSpan: 4,
                ...
                [
                    ...bukitBarisSub.flatMap(bb => [
                        { content: 'Kary', styles: { fontStyle: 'bold', textColor: [255, 255, 255] } },
                        { content: 'Note', styles: { fontStyle: 'bold', textColor: [255, 255, 255] } },
                        { content: 'Ast/Askep', styles: { fontStyle: 'bold', textColor: [255, 255, 255] } },
                        { content: 'EM/RC', styles: { fontStyle: 'bold', textColor: [255, 255, 255] } }
                    ])
                ]
            ];
  ...
  `;
        ...
        document.getElementById('exportPlacemarkKML').onclick = function () {
            ...
            let kml =
                `<?xml version="1.0" encoding="UTF-8"?><kml xmlns="http://www.opengis.net/kml/2.2"><Document><name>Placemarks TO</name>`;
            placemarks.forEach((p, i) => {
